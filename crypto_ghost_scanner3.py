"""
Crypto Ghost Scanner 3 (S3) -- Precision Filter Test
====================================================
Fork of Scanner 1 (v10.2) with three hardcoded precision upgrades.
All overrides are hardcoded -- this file does NOT read T2_MAX_ENTRY,
MIN_NO_PRICE, or MIN_TREND_STRENGTH from .env.

Purpose: run in parallel with Scanner 1 to measure whether stricter
filters produce a higher win rate on tier-2 (5-min Up/Down) markets.
All trades tagged strategy='s3' in DB for clean side-by-side comparison.

S3 overrides vs S1:
  T2_MAX_ENTRY       0.030 -> 0.010   only genuine stale orders (WR=14.6% vs 1.3%)
  MIN_NO_PRICE       0.97  -> 0.98    tighter certainty floor
  MIN_TREND_STRENGTH 0.001 -> 0.003   filters sideways/noise (was sub-0.1% noise level)
  + Exhaustion gate: if 1h trend disagrees with last 3 Binance ticks -> skip

PM2: pm2 start scanner3
DB tag: strategy='s3'  (S1 uses strategy='lottery')
"""
...
def log_trade(tier, coin, market_id, token_id, question, entry, size, order_id,
    ...
    """, (datetime.now(timezone.utc).isoformat(), tier, coin,
          market_id, token_id, question[:120], entry, size, order_id, outcome, trend_dev, secs_left, "s3",
...
class CryptoGhostScanner:
    ...
    def _print_banner(self):
        ...
        print(f"""
╔══════════════════════════════════════════════════════╗
║     CRYPTO GHOST SCANNER 3 -- PRECISION SNIPER       ║
║  {mode:<52s}║
╠══════════════════════════════════════════════════════╣
║  S3 overrides (hardcoded):                           ║
║    T2_MAX_ENTRY=0.010  MIN_NO_PRICE=0.98             ║
║    MIN_TREND_STRENGTH=0.003  + exhaustion gate       ║
╠══════════════════════════════════════════════════════╣
║  Wallet: {wallet:<44s}║
...
╚══════════════════════════════════════════════════════╝
    ...
    async def scan_updown_markets(self, session: aiohttp.ClientSession,
        ...
        async def check_market(m):
            ...
            MIN_TREND_STRENGTH = 0.003  # S3: raised from 0.001 — filters sideways noise
