...
_DB_PATH = os.path.join(
    ...
    "zugu_PAPER.db" if _PAPER_TRADE else "zugu.db",
)
    ...
    try:
        ...
        try:
            ...
            conn.execute(
                ...
                "entry_price, size_usdc, status, outcome, pm_result) "
                "VALUES (?,?,?,?,?,?,?,?, 'open', ?, 'pending')",
                (
                    ts,
                    FL_TIER,
...
def _evaluate_follow_leader(
    ...
    # Use Binance as the source of truth for the leading side (faster + cleaner
    # than the Polymarket oracle, which lags Binance by ~100-500ms).
    if bin_now > ptb:
        side = "UP"
    elif bin_now < ptb:
        side = "DOWN"
    ...
    lead_pct = abs(bin_now - ptb) / ptb
    if lead_pct < FL_MIN_LEAD_PCT:
        log.info(
            f"SKIP FL {asset} reason=lead_too_small "
            f"ptb={ptb:.4f} bin={bin_now:.4f} lead={lead_pct:.6f} "
            f"min={FL_MIN_LEAD_PCT:.6f} secs_left={secs_left}"
        )
        return None, None, "lead_too_small"
    ...
    if cur is not None and t.current_price_status == "ok":
        ...
        if pm_side is not None and pm_side != side:
            log.info(
                ...
                f"ptb={ptb:.4f} pm={cur:.4f} bin={bin_now:.4f} "
    ...
    log.info(
        ...
        f"cost={FL_FIXED_SHARES * ask:.2f} ptb={ptb:.4f} "
        f"bin={bin_now:.4f} lead={lead_pct:.6f} secs_left={secs_left}"
    )
