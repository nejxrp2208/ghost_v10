...
DB_PATH     = os.path.join(SCRIPT_DIR,
                "zugu_PAPER.db" if PAPER_TRADE else "zugu.db")
...
async def resolve_via_polymarket(session, token_id: str, market_id: str,
    ...
    try:
        ...
        if token_id:
            for t in tokens:
                if str(t.get("token_id", "")) == str(token_id):
                    ...
                    if price < 0.01:
                        return "lost", price
                    return "pending", None  # market closed but winner not yet posted
        ...
        # All tokens present but none marked winner yet — still settling
        return "pending", None
    except Exception:
        return "pending", None
