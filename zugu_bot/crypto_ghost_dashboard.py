...
def log_event(icon: str, msg: str) -> None:
    try:
        conn = db_connect()
...
# ── PM Oracle ─────────────────────────────────────────────────────────────────
async def _query_pm_oracle(session, market_id, token_id, our_outcome):
    """
    Checks PM settlement for a trade.
    - condition_id (hex, 64+ chars) → CLOB API, matched by token_id
    - numeric market_id             → gamma API (legacy fallback)
    """
    if not market_id:
        return None
    try:
        mid_str = str(market_id).strip()
        is_cid  = (not mid_str.isdigit()) and len(mid_str) > 20

        if is_cid:
            url = f"https://clob.polymarket.com/markets/{mid_str}"
            ...
            if not tokens:
                return None
            # Match by token_id when available
            if token_id:
                for t in tokens:
                    if str(t.get("token_id", "")) == str(token_id):
                        if t.get("winner"):
                            return "won"
                        if float(t.get("price", 1)) < 0.01:
                            return "lost"
                        return None  # market not yet settled
            # Fallback: find winner token and map outcome to UP/DOWN
            for t in tokens:
                if t.get("winner"):
                    ...
                    return "won" if our_up == win_up else "lost"
            return None
        else:
...
async def pm_oracle_checker():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        while True:
            ...
            try:
                rows = safe_query("""
                    SELECT id, market_id, COALESCE(token_id,''), outcome,
                           status, size_usdc, entry_price, question
                    FROM trades
                    WHERE status IN ('won','lost')
                      AND COALESCE(pm_result,'pending') = 'pending'
                      AND closed_at > datetime('now','-3 hours')
                """)
                for tid, market_id, token_id, outcome, status, size_usdc, entry_price, question in rows:
                    pm = await _query_pm_oracle(session, market_id, token_id, outcome)
                    if pm is None:
                        continue
                    if pm != status:
                        ...
                        log_event("⚠️",
                                  f"CORR #{tid} {coin} {direction}: "
                                  f"CL={status} → PM={pm} (${delta:+.2f})")
                        msg = (
                            f"⚠️ <b>Oracle Correction — Trade #{tid}</b>\n"
                            f"{direction} {question or 'N/A'}\n\n"
...
def render_last_trades():
    ...
    for rid, ts, tier, coin, q, size, pnl, status, outcome, pm_result, market_id, entry_price in rows:
        ...
        etxt = Text(f"{entry_price:.3f}", style="cyan") if entry_price else Text("—", style="dim white")
...
def render_activity():
    ...
    if rows:
        for ts, icon, msg in rows:
            ...
            mc = ("green"  if icon == "✅" or "WON" in msg_upper
                  else "red"    if icon == "❌" or "LOST" in msg_upper
                  else "yellow" if icon == "⚠️" or "OVERRIDE" in msg_upper or "CORR" in msg_upper
                  else "cyan"   if icon == "🎯"
                  else "yellow" if icon == "📋"
                  else "white")
