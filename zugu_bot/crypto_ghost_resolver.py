...
class _ChainlinkResolver:
    ...
    RETRY_INTERVAL = 60  # retry failed RPC connection every 1 minute
    ...
    def _connect(self):
        now = time.time()
        if self.tried and (self.connected or now - self.last_try < self.RETRY_INTERVAL):
...
async def resolve_via_polymarket(session, token_id: str, market_id: str,
                                our_outcome: str = ""):
    """
    Authoritative resolution via Polymarket CLOB API.
    Returns ('won'|'lost'|'pending', settled_value or None).
    Uses CLOB API with condition_id (market_id) — gamma-api archives
    5-minute markets too quickly and returns empty results.

    Resolution priority:
      1. Match by token_id (exact, most reliable)
      2. Find winner token by winner=True flag + map outcome to our direction
    """
    if not market_id:
        return "pending", None
    try:
        ...
        # 1. Match by token_id — works whether market is closed or not
        if token_id:
            for t in tokens:
                if str(t.get("token_id", "")) == str(token_id):
                    ...
                    if price < 0.01:
                        return "lost", price
                    # Market officially open but price near certainty (>99%) —
                    # PM sometimes delays closing for minutes after candle end.
                    # After 5 minutes we trust the price as the settled outcome.
                    if not data.get("closed") and price < 0.99:
                        return "pending", None
                    if price >= 0.99:
                        return "won", price   # our token is near-certain winner
                    return "pending", None

        # 2. Fallback: find winner token and map to our direction
        for t in tokens:
            if t.get("winner"):
                price  = float(t.get("price", 0))
                w_out  = str(t.get("outcome", "")).upper()
                our    = (our_outcome or "").upper()
                our_up = our in ("UP", "YES")
                win_up = w_out in ("UP", "YES")
                return ("won" if our_up == win_up else "lost"), price

        # No winner yet — check price certainty fallback
        if token_id:
            for t in tokens:
                if str(t.get("token_id", "")) == str(token_id):
                    price = float(t.get("price", 0))
...
async def resolve_via_polymarket_source(session, coin: str, start_utc, end_utc, bought_up: bool):
    ...
    if (now - end_utc).total_seconds() < 15:
...
async def verify_against_pm_gamma(session, trade_id: int, token_id: str,
    ...
    for attempt in range(PM_VERIFY_RETRY):
        try:
            pm_status, pm_settled = await resolve_via_polymarket(
                session, token_id, market_id, side
            )
        except Exception as e:
            print(f"[PM-VERIFY] #{trade_id} CLOB error: {e}")
            pm_status = "pending"
    ...
    if pm_status == local_status:
        # Match — stamp verification code + pm_result so dashboard skips re-check
        try:
            conn = db_connect()
            conn.execute("""
                UPDATE trades SET resolution_verified = ?, pm_result = ?
                WHERE id = ? AND status = ?
            """, (RESOLUTION_CODE["pm_gamma_confirm"], pm_status, trade_id, local_status))
            conn.commit()
            conn.close()
    ...
    try:
        ...
        conn.execute("""
            UPDATE trades
            SET status = ?, pnl = ?,
                resolution_verified = ?, pm_result = ?
            WHERE id = ?
        """, (pm_status, new_pnl,
              RESOLUTION_CODE["pm_gamma_override"], pm_status, trade_id))
        conn.commit()
        conn.close()
            ...
            try:
                ...
                if not open_trades:
                    ...
                    for trade in open_trades:
                        ...
                        # ── PRIMARY: Polymarket Oracle (authoritative winner flag) ──
                        # Wait 15s minimum for PM to mark the market closed, then poll
                        # until winner=True appears. No Chainlink — no wrong intermediates.
                        if secs_after < 15:
                            print(f"[{ts}] ⏳ T{tier} {coin} #{trade_id} {side} | "
                                  f"closed {secs_after:.0f}s ago, waiting for Oracle")
                            continue

                        status, settled_price = await resolve_via_polymarket(
                            session, token_id, market_id, outcome
                        )

                        if status == "pending":
                            print(f"[{ts}] ⏳ T{tier} {coin} #{trade_id} {side} | "
                                  f"Oracle settling ({secs_after:.0f}s after close)…")
                            continue
                        ...
                        close_trade(trade_id, status, pnl, None, settled_price,
                                    cid, source="pm_oracle")

                        price_str = f"settled={settled_price:.4f}" if settled_price else "(price n/a)"
                        tg_price  = f"{settled_price:.4f}" if settled_price else ""
                        print(f"[{ts}] {icon} T{tier} {coin} #{trade_id} {side} | "
                              f"{status.upper()} ${pnl:+.2f} [PM] | {price_str}")
                        if cid:
